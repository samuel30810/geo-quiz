...
這是一個純前端靜態地理練習網站，目標使用者為學生（國小為主）。系統提供台灣縣市（22 個）與世界國家（約 50 個）兩種地圖模式，無需建置步驟，直接以瀏覽器開啟即可使用。練習模式為：地圖隨機高亮一個區域並自動 zoom 進去，使用者從四個文字選項中選出正確名稱，作答後即時顯示回饋。介面以手機優先設計，部署於 GitHub Pages，無後端、無登入，所有狀態存在記憶體，重整後重置。
...
| 台灣資料設定 | 台灣縣市 TopoJSON URL、屬性欄位、名稱取得方式 | `js/data-taiwan.js` | 實作 MAP_CONFIG 介面 |
