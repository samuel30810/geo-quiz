...
這是一個純前端靜態地理練習網站，目標使用者為學生（國小為主）。系統提供台灣縣市與離島（22 縣市 + 5 離島，共 27 個）與世界國家（約 50 個）兩種地圖，以及兩種練習模式：**練習模式**（題庫全出一次不重複，結束後顯示正確率圓環）和**隨機模式**（隨機抽 20 題不重複，一題 5 分滿分 100，結束後顯示分數）。首頁為兩步驟流程：先選模式、再選地圖。作答流程為：地圖高亮一個區域並自動 zoom 進去，使用者從四個文字選項中選出正確名稱，作答後即時顯示回饋，全部答完後顯示結束畫面。介面以手機優先設計，部署於 GitHub Pages，無後端、無登入，所有狀態存在記憶體，重整後重置。
