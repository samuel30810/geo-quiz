...
## 2026-05-21 — 世界地圖選項按鈕加入國家介紹文字

**觸發情境**：使用者希望選項按鈕直接顯示中文名、英文名、國家介紹三層內容，不用等到作答後才看到介紹。
**根因**：非 bug，為 UX 功能新增。
**修改檔案**：

- `js/engine.js`：`buildChoices` 新增 `desc` 欄位（呼叫 `config.getDesc`，超過 45 字或無資料則為空字串）；`renderChoices` 條件渲染 `.choice-desc` span。
- `css/style.css`：新增 `.choice-desc`（10px、`-webkit-line-clamp:2`、left-align）。

**編譯結果**：✅ 純 JS，無編譯步驟

**文件更新**：更新了 `docs/architecture.md`（系統概述補充選項按鈕行為；關鍵決策新增「選項按鈕顯示國家介紹」）；更新了 `README.md`（功能列表「國家介紹」描述改為選項中直接顯示）
...
- `index.html`：Step 1 內容由模式選擇卡改為地圖選擇卡（含地圖預覽）；Step 2 內容由地圖選擇卡改為模式選擇卡；JS 重新對應新流程（`selectedMap` 取代 `selectedMode`，Step 2 模式按鈕點擊觸發跳轉或難度 overlay）；難度 overlay 邏輯不變（世界 + 隨機時顯示）；練習卡副標動態顯示所選地圖的實際題數。

**編譯結果**：✅ 純 JS，無編譯步驟；瀏覽器實測三條路徑均正確（台灣練習、台灣隨機、世界練習、世界隨機→難度選擇）
